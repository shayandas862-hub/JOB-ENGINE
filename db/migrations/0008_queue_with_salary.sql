-- 0008_queue_with_salary.sql
-- Applied via Supabase MCP (apply_migration: v2_queue_with_salary).
-- Rebuilds the view chain so the apply queue carries the advisory salary wall,
-- with thresholds read from my_constraints (so changing them there updates the wall).

drop view if exists v_skill_gap;
drop view if exists v_skill_demand;
drop view if exists v_apply_queue;

create view v_apply_queue as
select
